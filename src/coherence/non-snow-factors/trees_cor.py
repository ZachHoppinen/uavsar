...
tmp_dir = expanduser(f'~/uavsar/results/non-snow-coh/tmp/')
...
def process(img_set):
    ...
    inc.name = 'inc'
    dem = rxa.open_rasterio(img_set['hgt'])
    dem.name = 'dem'
    ...
    phase = phase.rio.reproject_match(cor_re)
    inc = inc.rio.reproject_match(cor_re)
    dem = dem.rio.reproject_match(cor_re)
    
    merged = xa.merge([phase,cor,inc, trees])
...
with open(expanduser(f'~/uavsar/results/non-snow-coh/resv1'), 'wb') as f:
    pickle.dump(res, f)
