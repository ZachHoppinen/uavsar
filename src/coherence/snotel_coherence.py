...
#             res = pd.concat([res, pd.DataFrame.from_records([dic])])
# res['diff_dt'] = (res['second_dt'] - res['first_dt']).astype('timedelta64[D]')
# with open(expanduser(f'~/uavsar/results/coh_snotel_temp/res_df_mean'), 'wb') as f:
#     pickle.dump(res, f)
...
for dir in glob(join(data_dir, '*')):
    ...
    for pair in tqdm(pairs, desc = basename(dir)):
        ...
        for coh_fp in coh_fps:
            ...
            res = pd.concat([res, pd.DataFrame.from_records([dic])])
res['diff_dt'] = (res['second_dt'] - res['first_dt']).astype('timedelta64[D]')
with open(expanduser(f'~/uavsar/results/coh_snotel_temp/res_df_dmd'), 'wb') as f:
    pickle.dump(res, f)
...
for dir in glob(join(data_dir, '*')):
    ...
    for pair in tqdm(pairs, desc = basename(dir)):
        ...
        for coh_fp in coh_fps:
            ...
            res = pd.concat([res, pd.DataFrame.from_records([dic])])
res['diff_dt'] = (res['second_dt'] - res['first_dt']).astype('timedelta64[D]')
with open(expanduser(f'~/uavsar/results/coh_snotel_temp/res_df_diff'), 'wb') as f:
    pickle.dump(res, f)